...
def get_most_no1_artists(df, agg = 'day'):
    artist_daily_plays = df.groupby(['date', 'artist'])['minutes'].sum().reset_index()
...
def get_most_no1_songs(df, agg = 'day'):
    song_daily_plays = df.groupby(['date', 'track', 'artist'])['minutes'].sum().reset_index()
    most_streamed_per_day = song_daily_plays.loc[song_daily_plays.groupby('date')['minutes'].idxmax()]
    most_streamed_per_day_count = most_streamed_per_day.groupby(['track', 'artist']).size().sort_values(ascending=False).reset_index().rename(columns={0: 'days at 1'})
